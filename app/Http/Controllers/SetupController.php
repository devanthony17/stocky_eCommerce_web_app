...
class SetupController extends Controller
{
    ...
    public function lastStep(Request $request)
    {
        ...
        try {
            // 1) Clear any old config cache
            Artisan::call('config:clear');

            // 2) Drop all tables, re-run all app migrations + seeders
            //    (Passport's oauth_* tables are already included in the app migrations)
            Artisan::call('migrate:fresh', [
                '--force' => true,
                '--seed'  => true,
            ]);

            // 3) Generate Passport keys & create clients without interactive prompts
            ...
            // 4) Mark the install as done
            Storage::disk('public')->put('installed', 'OK');

            // 5) Write .env LAST — this triggers `php artisan serve` to restart,
            //    so everything above must finish before this line.
            $this->changeEnv([
